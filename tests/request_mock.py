...
class RequestMock(object):
    def __init__(self, mocker):
        ...
        self.constructor_patcher = self._mocker.patch(
            "octane.api_requestor.APIRequestor.__init__",
            side_effect=octane.api_requestor.APIRequestor.__init__,
            autospec=True,
        )

        self.request_patcher = self._mocker.patch(
            "octane.api_requestor.APIRequestor.request",
...
class StubRequestHandler(object):
    ...
    def get_response(self, method, url):
        if (method, url) in self._entries:
            ...
            octane_response = OctaneResponse(rbody, rcode, rheaders)
            return octane_response
