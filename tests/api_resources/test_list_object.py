...
class TestListObject(object):
    ...
    def list_object(self):
        return octane.ListObject.construct_from(
    ...
    def test_list(self, request_mock, list_object):
        ...
        assert isinstance(res, octane.ListObject)
        assert isinstance(res.data, list)
        assert isinstance(res.data[0], octane.Charge)
    ...
    def test_create(self, request_mock, list_object):
        ...
        assert isinstance(res, octane.Charge)
        assert res.foo == "bar"
    ...
    def test_create_maintains_list_properties(self, request_mock, list_object):
        ...
        # test otherwise
        charge = octane.Charge.retrieve("ch_123", api_key="sk_test_custom")
    ...
    def test_is_empty(self):
        lo = octane.ListObject.construct_from({"data": []}, None)
        assert lo.is_empty is True

    def test_empty_list(self):
        lo = octane.ListObject.empty_list()
        assert lo.is_empty

    def test_iter(self):
        arr = [{"id": 1}, {"id": 2}, {"id": 3}]
        expected = octane.util.convert_to_octane_object(arr)
        lo = octane.ListObject.construct_from({"data": arr}, None)
        assert list(lo) == expected

    def test_iter_reversed(self):
        arr = [{"id": 1}, {"id": 2}, {"id": 3}]
        expected = octane.util.convert_to_octane_object(list(reversed(arr)))
        lo = octane.ListObject.construct_from({"data": arr}, None)
    ...
    def test_next_page_empty_list(self):
        ...
        next_lo = lo.next_page()
        assert next_lo == octane.ListObject.empty_list()
    ...
    def test_serialize_empty_list(self):
        ...
        deserialized = octane.ListObject.construct_from(
    ...
    def test_serialize_nested_empty_list(self):
        ...
        serialized = str(obj)
        deserialized = octane.octane_object.OctaneObject.construct_from(
            json.loads(serialized), "mykey"
        )
...
class TestAutoPaging:
    ...
    def test_class_method_two_pages(self, request_mock):
        ...
        seen = [
            item["id"]
            for item in octane.Charge.auto_paging_iter(limit=25, foo="bar")
        ]
