...
class TestAPIResource(object):
    class MyResource(octane.api_resources.abstract.APIResource):
    ...
    def test_convert_to_octane_object(self):
        sample = {
            "foo": "bar",
            "adict": {"object": "customer", "name": "chilango"},
            "alist": [{"object": "customer", "name": "chilango"}],
        }

        converted = octane.util.convert_to_octane_object(sample, "akey", None)

        # Types
        assert isinstance(converted, octane.octane_object.OctaneObject)
        assert isinstance(converted.adict, octane.Customer)
        assert len(converted.alist) == 1
        assert isinstance(converted.alist[0], octane.Customer)

        # Values
        assert converted.foo == "bar"
        assert converted.adict.name == "chilango"
