...
from octane import six, error


class TestOctaneError(object):
    def test_formatting(self):
        err = error.OctaneError(u"öre")
    ...
    def test_formatting_with_request_id(self):
        err = error.OctaneError(u"öre", headers={"request-id": "123"})
    ...
    def test_formatting_with_none(self):
        err = error.OctaneError(None, headers={"request-id": "123"})
    ...
    def test_repr(self):
        ...
        if six.PY2:
            assert (
                repr(err)
                == "OctaneError(message=u'\\xf6re', http_status=None, "
                "request_id='123')"
            )
        else:
            assert (
                repr(err) == "OctaneError(message='öre', http_status=None, "
                "request_id='123')"
            )

    def test_error_object(self):
        err = error.OctaneError(
    ...
    def test_error_object_not_dict(self):
        err = error.OctaneError("message", json_body={"error": "not a dict"})
        assert err.error is None


class TestOctaneErrorWithParamCode(object):
    def test_repr(self):
        err = error.CardError(
            u"öre",
            param="cparam",
