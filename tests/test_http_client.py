...
class OctaneClientTestCase(object):
    ...
    def request_mocks(self, mocker):
        ...
        for lib in self.REQUEST_LIBRARIES:
            request_mocks[lib] = mocker.patch("octane.http_client.%s" % (lib,))
        return request_mocks


class TestNewDefaultHttpClient(OctaneClientTestCase):
    @pytest.fixture(autouse=True)
    def setup_warnings(self, request_mocks):
        original_filters = octane.http_client.warnings.filters[:]
        octane.http_client.warnings.simplefilter("ignore")
        yield
        octane.http_client.warnings.filters = original_filters

    def check_default(self, none_libs, expected):
        for lib in none_libs:
            setattr(octane.http_client, lib, None)

        inst = octane.http_client.new_default_http_client()

        assert isinstance(inst, expected)

    def test_new_default_http_client_urlfetch(self):
        self.check_default((), octane.http_client.UrlFetchClient)

    def test_new_default_http_client_requests(self):
        self.check_default(("urlfetch",), octane.http_client.RequestsClient)

    def test_new_default_http_client_pycurl(self):
        self.check_default(
            ("urlfetch", "requests"), octane.http_client.PycurlClient
        )

    def test_new_default_http_client_urllib2(self):
        self.check_default(
            ("urlfetch", "requests", "pycurl"),
            octane.http_client.Urllib2Client,
        )


class TestRetrySleepTimeDefaultHttpClient(OctaneClientTestCase):
    ...
    def mock_max_delay(self, new_value):
        original_value = octane.http_client.HTTPClient.MAX_DELAY
        octane.http_client.HTTPClient.MAX_DELAY = new_value
        try:
            yield self
        finally:
            octane.http_client.HTTPClient.MAX_DELAY = original_value

    def test_sleep_time_exponential_back_off(self):
        client = octane.http_client.new_default_http_client()
    ...
    def test_initial_delay_as_minimum(self):
        ...
        initial_delay = octane.http_client.HTTPClient.INITIAL_DELAY
        self.assert_sleep_times(client, [initial_delay] * 5)

    def test_maximum_delay(self):
        client = octane.http_client.new_default_http_client()
        client._add_jitter_time = lambda t: t
        max_delay = octane.http_client.HTTPClient.MAX_DELAY
    ...
    def test_randomness_added(self):
        ...
        base_value = octane.http_client.HTTPClient.INITIAL_DELAY * random_value

        with self.mock_max_delay(10):
            expected = [
                octane.http_client.HTTPClient.INITIAL_DELAY,
                base_value * 2,
                base_value * 4,
                base_value * 8,
                base_value * 16,
...
class TestRetryConditionsDefaultHttpClient(OctaneClientTestCase):
    ...
    def test_should_retry_on_octane_should_retry_true(self, mocker):
        client = octane.http_client.new_default_http_client()
        client._max_network_retries = lambda: 1
        headers = {"octane-should-retry": "true"}
    ...
    def test_should_retry_on_octane_should_retry_false(self, mocker):
        client = octane.http_client.new_default_http_client()
        client._max_network_retries = lambda: 1
        headers = {"octane-should-retry": "false"}
...
class TestHTTPClient(object):
    ...
    def setup_octane(self):
        orig_attrs = {"enable_telemetry": octane.enable_telemetry}
    ...
    def test_sends_telemetry_on_second_request(self, mocker):
        class TestClient(octane.http_client.HTTPClient):
            pass
        ...
        assert "X-Octane-Client-Telemetry" in args[2]

        telemetry = json.loads(args[2]["X-Octane-Client-Telemetry"])
...
class ClientTestBase(object):
    ...
    def valid_url(self, path="/foo"):
        return "https://api.cloud.getoctane.io%s" % (path,)
...
class RequestsVerify(object):
    def __eq__(self, other):
        return other and other.endswith("octane/data/ca-certificates.crt")


class TestRequestsClient(OctaneClientTestCase, ClientTestBase):
    REQUEST_CLIENT = octane.http_client.RequestsClient
...
class TestRequestClientRetryBehavior(TestRequestsClient):
    ...
    def connection_error(self, session):
        ...
        def connection_error(given_exception):
            with pytest.raises(octane.error.APIConnectionError) as error:
    ...
    def test_handle_request_error_should_not_retry(
        ...
        assert "not verify Octane's SSL certificate" in error.user_message
...
class TestUrlFetchClient(OctaneClientTestCase, ClientTestBase):
    REQUEST_CLIENT = octane.http_client.UrlFetchClient
...
class TestUrllib2Client(OctaneClientTestCase, ClientTestBase):
    REQUEST_CLIENT = octane.http_client.Urllib2Client
...
class TestPycurlClient(OctaneClientTestCase, ClientTestBase):
    REQUEST_CLIENT = octane.http_client.PycurlClient
    ...
    def bio_getvalue(self, mocker):
        bio_patcher = mocker.patch("octane.util.io.BytesIO")
    ...
    def mock_error(self):
        def mock_error(mock):
            ...
            octane.http_client.pycurl.error = FakeException
            mock.perform.side_effect = octane.http_client.pycurl.error
    ...
    def check_call(self, request_mocks):
        def check_call(mock, method, url, post_data, headers):
            ...
            mock.setopt.assert_any_call(lib_mock.URL, octane.util.utf8(url))
...
class TestAPIEncode(OctaneClientTestCase):
    def test_encode_dict(self):
        body = {"foo": {"dob": {"month": 1}, "name": "bat"}}

        values = [t for t in octane.api_requestor._api_encode(body)]
