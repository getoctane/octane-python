...
from octane.six.moves.urllib.parse import parse_qsl


OCTANE_LOG = os.environ.get("OCTANE_LOG")
...
def _console_log_level():
    if octane.log in ["debug", "info"]:
        return octane.log
    elif OCTANE_LOG in ["debug", "info"]:
        return OCTANE_LOG
    else:
        return None
...
def _test_or_live_environment():
    if octane.api_key is None:
        return
    match = re.match(r"sk_(live|test)_", octane.api_key)
    if match is None:
        return
...
def dashboard_link(request_id):
    return "https://dashboard.cloud.getoctane.io/{env}/logs/{reqid}".format(
...
def get_object_classes():
    ...
    from octane.object_classes import OBJECT_CLASSES

    return OBJECT_CLASSES


def convert_to_octane_object(resp, api_key=None, octane_version=None):
    # If we get a OctaneResponse, we'll want to return a
    # OctaneObject with the last_response field filled out with
    # the raw API response information
    octane_response = None

    if isinstance(resp, octane.octane_response.OctaneResponse):
        octane_response = resp
        resp = octane_response.data

    if isinstance(resp, list):
        return [
            convert_to_octane_object(i, api_key, octane_version) for i in resp
        ]
    elif isinstance(resp, dict) and not isinstance(
        resp, octane.octane_object.OctaneObject
    ):
        resp = resp.copy()
        klass_name = resp.get("object")
        if isinstance(klass_name, six.string_types):
            klass = get_object_classes().get(
                klass_name, octane.octane_object.OctaneObject
            )
        else:
            klass = octane.octane_object.OctaneObject

        return klass.construct_from(
            resp,
            api_key,
            octane_version=octane_version,
            last_response=octane_response,
        )
    else:
        return resp
