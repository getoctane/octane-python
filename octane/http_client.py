...
import octane
from octane import error, util, six
from octane.request_metrics import RequestMetrics
...
try:
    from octane.six.moves import urllib
except ImportError:
...
try:
    ...
    try:
        ...
        if (major, minor, patch) < (0, 8, 8):
            sys.stderr.write(
                "Warning: the Octane library requires that your Python "
                '"requests" library be newer than version 0.8.8, but your '
                '"requests" library is version %s. Octane will fall back to '
                "an alternate HTTP library so everything should work. We "
                'recommend upgrading your "requests" library. If you have any '
                "questions, please contact support@getoctane.io. (HINT: running "
...
from octane.six.moves.urllib.parse import urlparse
...
def new_default_http_client(*args, **kwargs):
    if urlfetch:
        ...
        impl = Urllib2Client
        warnings.warn(
            "Warning: the Octane library is falling back to urllib2/urllib "
...
class HTTPClient(object):
    ...
    def _should_retry(self, response, api_connection_error, num_retries):
        ...
        if rheaders is not None and "octane-should-retry" in rheaders:
            if rheaders["octane-should-retry"] == "false":
                return False
            if rheaders["octane-should-retry"] == "true":
                return True
        ...
        #
        # Note that we expect the octane-should-retry header to be false
    ...
    def _max_network_retries(self):
        from octane import max_network_retries
    ...
    def _add_telemetry_header(self, headers):
        ...
        if octane.enable_telemetry and last_request_metrics:
            telemetry = {
                "last_request_metrics": last_request_metrics.payload()
            }
            headers["X-Octane-Client-Telemetry"] = json.dumps(telemetry)

    def _record_request_metrics(self, response, request_start):
        _, _, rheaders = response
        if "Request-Id" in rheaders and octane.enable_telemetry:
...
class RequestsClient(HTTPClient):
    ...
    def request(self, method, url, headers, post_data=None):
        ...
        if self._verify_ssl_certs:
            kwargs["verify"] = octane.ca_bundle_path
        else:
            kwargs["verify"] = False
        ...
        try:
            try:
                ...
                raise TypeError(
                    ...
                    '"requests" library is not compatible with Octane\'s '
    ...
    def _handle_request_error(self, e):
        ...
        if isinstance(e, requests.exceptions.SSLError):
            msg = (
                "Could not verify Octane's SSL certificate.  Please make "
                "sure that your network is not intercepting certificates.  "
                "If this problem persists, let us know at "
                "support@getoctane.io."
            )
...
class UrlFetchClient(HTTPClient):
    ...
    def _handle_request_error(self, e, url):
        if isinstance(e, urlfetch.InvalidURLError):
            msg = (
                "The Octane library attempted to fetch an "
                "invalid URL (%r). This is likely due to a bug "
                "in the Octane Python bindings. Please let us know "
                "at support@getoctane.io." % (url,)
            )
            ...
            msg = (
                ...
                "Please let us know at support@getoctane.io."
            )
        else:
            msg = (
                "Unexpected error communicating with Octane. If this "
                "problem persists, let us know at support@getoctane.io."
            )
...
class PycurlClient(HTTPClient):
    ...
    def _handle_request_error(self, e):
        if e.args[0] in [
            ...
            msg = (
                "Could not connect to Octane.  Please check your "
...
class Urllib2Client(HTTPClient):
    ...
    def _handle_request_error(self, e):
        msg = (
            "Unexpected error communicating with Octane. "
            "If this problem persists, let us know at support@getoctane.io."
        )
        msg = textwrap.fill(msg) + "\n\n(Network error: " + str(e) + ")"
        raise error.APIConnectionError(msg)
